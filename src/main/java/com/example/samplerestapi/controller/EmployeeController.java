...
public class EmployeeController {
    ...
    record EmployeeDto(int id, @JsonProperty("first_name") String firstName,
    ...
    public ResponseEntity<EmployeeDto> update(@RequestBody EmployeeDto employeeDto) {
        var employee = new Employee(employeeDto.firstName,
                employeeDto.lastName,
                employeeDto.email);
        employee.setId(employeeDto.id);
        return ResponseEntity
                .ok()
                .body(toDto(employeeDao.saveAndFlush(employee)));
    }
    ...
    public List<EmployeeDto> deleteEmployee(@PathVariable("id") int employeeId) {
        if (employeeDao.existsById(employeeId)) {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND
                    , employeeId + "Not Found in Employee list");
        return employeeDao
                .findAll()
                .stream()
                .map(this::toDto)
